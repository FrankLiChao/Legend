...
//        [MBProgressHUD showSuccess:@"保存失败" toView:nil];
        [FrankTools showAlertWithMessage:@"保存失败" withSuperView:[UIApplication sharedApplication].keyWindow withHeih:DeviceMaxHeight/2];
    } else {
        MJPhoto *photo = _photos[_currentPhotoIndex];
        photo.save = YES;
        _saveImageBtn.enabled = NO;
        [FrankTools showAlertWithMessage:@"保存到相册成功" withSuperView:[UIApplication sharedApplication].keyWindow withHeih:DeviceMaxHeight/2];
    }
}
